...
def Linear_Regression():
    ...
    bonus = True
    if(bonus):
        sns.histplot(price_df, x=price_df['km'], element="step")
        plt.show()
        sns.scatterplot(data=price_df,  x=price_df['km'], y="Defense Against the Dark Arts", hue="Hogwarts House", style="Hogwarts House")
        plt.show()
    return
    X_train = price_df['km'].to_numpy()
    X_test = price_df['price'].to_numpy()

    X_train, X_test, mean, std = standardize_data(X_train, X_test)
    theta0, theta1 = fit(X_train, X_test)
