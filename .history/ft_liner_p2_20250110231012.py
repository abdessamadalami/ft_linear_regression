...
def Linear_Regression():
    ...
    theta0, theta1 = fit(X_train_normalized, Y_train_normalized)
    # y = theta0 + theta1 * 2400
    theta1 = theta1 * (Y_max - Y_min) / (X_max - X_min)
    theta0_dentheta1orm = Y_min + theta0 * (Y_max - Y_min) - theta1_denorm * X_min

    print("Denormalized Theta1 (Slope):", theta1_denorm)
    print("Denormalized Theta0 (Intercept):", theta0_denorm)
    
    X_train_normalized = (240000 - X_min) / (X_max - X_min)
    # print(X_train_normalized)
    # y_predict_original = y * (Y_max - Y_min) + Y_min
    y = theta0 + (theta1 * X_train_normalized)
    print(X_train_normalized)
    print(y)
